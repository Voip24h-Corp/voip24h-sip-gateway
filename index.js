...
function StatusToOutSide(callbackToOutSide) {
...
function initGateWay(callbacks2) {
	callbacks2 = typeof (callbacks2) == "function" ? callbacks2 : () => { };
	Janus.init({
		debug: "all", callback: function () {
			if (!Janus.isWebrtcSupported()) {
				callbacks2('isWebrtcSupported', [{ 'message': "No WebRTC support..." }]);
				return;
			}
			let iOS = ['iPad', 'iPhone', 'iPod'].indexOf(navigator.platform) >= 0;
			let eventName = iOS ? 'pagehide' : 'beforeunload';
			...
			janus = new Janus(
				{
								...
								onmessage: function (msg, jsep) {
									...
									if (result && result["event"]) {
										...
										if (event === 'registration_failed') {
											...
											callbacks2('register', "registration_failed");
											return;
										}
										if (event === 'registered') {
											...
											callbacks2('register', 'registered_success');
										} else if (event === 'calling') {
											
											callbacks2('calling', ["calling", { 'callId': callId }]);
											...
											callbacks2('incomingcall', [{ 'phonenumber': phoneNumber }, { 'callid': callId }]);
											...
											if (jsep) {
												sipcall.handleRemoteJsep({ jsep: jsep, error: hangUp });
											}
											callbacks2('progress', "Connecting...");
											...
											callbacks2('accepted', "accepted_the_call");
											sipcall.callId = callId;
										} else if (event === 'transfer') {
											//Event transfer
											callbacks2('transfer', "transfering");
											...
											if (result["reason"] == "Busy Here") {
												callbacks2('reject', [{ "message": result["reason"] }]);
											} else if (result["reason"] == "to BYE") {
												callbacks2('hangup', "employer_hangup");
											} else if (result["reason"] == "Session Terminated") {
												callbacks2('hangup', "customer_hangup");
											} else {
												callbacks2('hangup', result["reason"]);
											}
											// Reset status
											sipcall.hangup();
										}
										if (event === 'holding') {
											checkHold = "holding";
											callbacks2('holding', "holding");
										} else {
											checkHold = "";
											callbacks2('holding', "unholding");
										}
									}
								},
					...
					error: function (error) {
						Janus.error(error);
						callbacks2('error', [{ "message": "Service failed " + error }]);
						if (error == "Lost connection to the server (is it down?)") {
							callbacks2('error', "server_down");
						}
					},
					destroyed: function () {
						...
						callbacks2('destroyed', "destroyed");
					}
				});
...
async function registerSip(ipSip, sip, secret) {
	var port = 5060;
	var sipserver = null;
	var username = null;
	var password = null;
...
function transfer(transferToNumber) {
...
module.exports = { initGateWay, registerSip, call, hangUp, answer, reject, toggleHold, toggleMute, transfer, sendDtmf, isHold, isMute, isRegistered, hasCheckDevice, release, StatusToOutSide }